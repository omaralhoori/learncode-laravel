...
@section('content')
    @include('admin.admins.partials.header', ['title' => __('Edit Quiz')])
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="name" id="input-name" class="form-control form-control-alternative{{ $errors->has('name') ? ' is-invalid' : '' }}"
                                           placeholder="{{ __('Name') }}" value="{{ $quiz->name }}" required autofocus>
                                    ...
                                    <select name="course_id" id="input-course" required class="form-control">
                                        @foreach(\App\Course::orderBy('id', 'desc')->get() as $course)
                                            <option value="{{$course->id}}"
                                                    <?php if($quiz->course->id == $course->id) echo 'selected';?>
