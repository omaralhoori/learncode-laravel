...
@section('content')
    @include('admin.admins.partials.header', ['title' => __('Edit Video')])
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('title') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="title" id="input-title" class="form-control form-control-alternative{{ $errors->has('title') ? ' is-invalid' : '' }}" placeholder="{{ __('Title') }}"
                                           value="{{ $video->title }}" required autofocus>
                                ...
                                <div class="form-group{{ $errors->has('link') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="link" id="input-link" class="form-control form-control-alternative{{ $errors->has('link') ? ' is-invalid' : '' }}" placeholder="{{ __('Link') }}"
                                ...
                                <div class="form-group{{ $errors->has('course_id') ? ' has-danger' : '' }}">
                                    ...
                                    <select name="course_id" id="input-course" required class="form-control">
                                        @foreach(\App\Course::orderBy('id', 'desc')->get() as $course)
                                            ...
                                            <?php if($video->course->id == $course->id) echo 'selected';?>
                                            >{{\Str::limit($course->title, 50)}}</option>
                                        @endforeach
                                    </select>
